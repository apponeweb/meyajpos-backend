...
-- Versión PostgreSQL (ambiente de desarrollo - multi-tenancy)
-- La vista se crea en el esquema tiosbarber

SET search_path TO tiosbarber, public;

CREATE OR REPLACE VIEW tiosbarber.vw_daily_report AS
SELECT
  sd.id AS detail_id,
  ...
  (SELECT COALESCE(SUM(t.amount),0) FROM tiosbarber.tbd_tip t
     WHERE t.sale_payment_id IN
       (SELECT sp2.id FROM tiosbarber.tbr_sale_payment sp2 WHERE sp2.sale_id = s.id)) AS tip_amount,
  sd.total AS total,
  (SELECT COALESCE(SUM(sp3.amount_received),0) FROM tiosbarber.tbr_sale_payment sp3
     WHERE sp3.sale_id = s.id) AS payment_amount,
  COALESCE(s.cash_change,0) AS cash_change,
  (SELECT pt2.name FROM tiosbarber.tbr_sale_payment sp4
     JOIN tiosbarber.tbn_payment_type pt2 ON sp4.payment_type_id = pt2.id
     WHERE sp4.sale_id = s.id LIMIT 1) AS payment_method,
  (SELECT sp5.payment_type_id FROM tiosbarber.tbr_sale_payment sp5
     WHERE sp5.sale_id = s.id LIMIT 1) AS payment_method_id,
  TO_CHAR(s.sale_date, 'DD/MM/YYYY HH24:MI:SS') AS formatted_sale_date,
  s.sale_date AS sale_date,
  COALESCE(cb.name,'N/A') AS cash_box_name,
  b.id AS branch_id,
  COALESCE(b.name,'') AS branch_name
FROM tiosbarber.tbd_sale_detail sd
JOIN tiosbarber.tbd_sale s ON sd.sale_id = s.id
LEFT JOIN tiosbarber.tbd_cash_box cb ON s.cash_box_id = cb.id
LEFT JOIN tiosbarber.tbn_branch b ON cb.branch_id = b.id
JOIN tiosbarber.tbd_master_product p ON sd.product_id = p.id
JOIN tiosbarber.tbn_service_type st ON p.service_type_id = st.id
LEFT JOIN tiosbarber."user" u ON sd.service_provider_id = u.id;

-- Versión MySQL (ambiente de producción - referencia)
